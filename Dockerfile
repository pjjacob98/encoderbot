...
RUN git clone https://git.ffmpeg.org/ffmpeg.git ; cd ffmpeg ; ./configure --enable-gpl --enable-libx264 --enable-libx265 --enable-libvpx ; make > /dev/null 2>&1 ; make install ; rm -rfv ../ffmpeg
RUN git clone https://github.com/sekrit-twc/zimg ; cd zimg ; ./autogen.sh ; ./configure ; make > /dev/null 2>&1 ; make install ; rm -rfv ../zimg
RUN git clone https://github.com/vapoursynth/vapoursynth.git ; cd vapoursynth ; ./autogen.sh ; ./configure ; make > /dev/null 2>&1 ; make install ; rm -rfv ../vapoursynth
