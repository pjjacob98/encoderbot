...
RUN git clone https://git.ffmpeg.org/ffmpeg.git ; cd ffmpeg ; ./configure --enable-gpl --enable-libx264 --enable-libx265 --enable-libvpx ; make > /dev/null 2>&1 ; make install
